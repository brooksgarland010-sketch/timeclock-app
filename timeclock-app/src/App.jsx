...
const getWeekDatesForOffset = (offset=0) => {
  ...
  return [0,3,4,5,6].map(o=>{ const d=new Date(fri); d.setDate(fri.getDate()+o); return d.toISOString().split('T')[0] })
}
...
export default function App() {
  ...
  const inputStyle={padding:'10px 13px',borderRadius:8,border:'0.5px solid #ccc',background:'#fff',color:'#111',fontSize:15,width:'100%',fontFamily:'inherit',boxSizing:'border-box'}

  if(!DB) return <div style={{padding:24,fontFamily:'system-ui'}}><h2>Firebase not configured</h2></div>
  if(view==='loading') return <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100dvh',fontFamily:'system-ui',color:'#666'}}>Loading…</div>
  ...
  if(view==='setup') return (
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'#F7F7F6',fontFamily:'system-ui',padding:24}}>
      ...
      <div style={{width:'100%',maxWidth:320,display:'flex',flexDirection:'column',gap:12}}>
        <input ref={setupUserInput} placeholder="Admin username" autoComplete="username" autoCapitalize="none" onChange={e=>setupUserRef.current=e.target.value} style={{padding:'12px 14px',borderRadius:10,border:'1px solid #ddd',fontSize:15,background:'#fff'}}/>
        <input ref={setupPassInput} placeholder="Password" type="password" autoComplete="new-password" onChange={e=>setupPassRef.current=e.target.value} onKeyDown={e=>e.key==='Enter'&&doSetupAdmin()} style={{padding:'12px 14px',borderRadius:10,border:'1px solid #ddd',fontSize:15,background:'#fff'}}/>
  ...
  if(view==='login') {
    ...
    return (
      <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'#F7F7F6',fontFamily:'system-ui',padding:24}}>
        ...
        <div style={{width:'100%',maxWidth:320,display:'flex',flexDirection:'column',gap:12}}>
          <input ref={loginUserInput} placeholder="Username" autoComplete="username" autoCapitalize="none" onChange={e=>loginUserRef.current=e.target.value} onKeyDown={e=>e.key==='Enter'&&loginPassInput.current?.focus()} style={{padding:'12px 14px',borderRadius:10,border:'1px solid #ddd',fontSize:15,background:'#fff'}}/>
          <input ref={loginPassInput} placeholder="Password" type="password" autoComplete="current-password" onChange={e=>loginPassRef.current=e.target.value} onKeyDown={e=>e.key==='Enter'&&doLogin()} style={{padding:'12px 14px',borderRadius:10,border:'1px solid #ddd',fontSize:15,background:'#fff'}}/>
          {loginError&&<div style={{fontSize:13,color:'#C0392B',textAlign:'center'}}>{loginError}</div>}
          <button onClick={doLogin} disabled={loginLoad} style={{padding:14,borderRadius:12,background:'#111',color:'#fff',border:'none',fontSize:15,fontWeight:600,cursor:'pointer',opacity:loginLoad?0.6:1}}>{loginLoad?'Signing in…':'Sign In'}</button>
          {bioUser&&<button onClick={()=>doBioLogin(bioUserId)} disabled={loginLoad} style={{padding:14,borderRadius:12,background:'#fff',color:'#111',border:'1px solid #ddd',fontSize:15,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}><span style={{fontSize:20}}>🔐</span>Use Face ID as {bioUser.username}</button>}
        </div>
      </div>
    )
  }
  ...
  const WeekRow = ({date, empId, forAdmin=false}) => {
    ...
    return (
      <div className={`week-row ${isToday?'today':dw>0?'worked':''}`} style={{display:'flex',alignItems:'center'}}>
        ...
        <button onClick={()=>openEdit(empId,date)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:15,padding:'2px 4px',lineHeight:1}}>
      ...
      <main className="page">
        ...
        {view==='home'&&isAdmin&&(
          <>
            ...
            <div style={{padding:'12px 16px',display:'flex',flexDirection:'column',gap:8}}>
              {employees.length===0?<div className="empty"><i className="ti ti-users" aria-hidden="true"/>No employees yet. Add them in the Employees tab.</div>:employees.map(emp=>{
          ...
          return(
            <>
              ...
              <div style={{padding:16}}>
                <div style={{marginBottom:18}}><div style={{fontSize:22,fontWeight:700,marginBottom:4}}>{myEmp.name}</div><span className="pill" style={{background:sc.bg,color:sc.color}}>{sc.label}</span></div>
          ...
          return(
            <>
              ...
              <div style={{padding:16}}>
                ...
                <div className="label">This week</div>
                <div style={{display:'flex',flexDirection:'column',gap:4}}>
                  {wd.map(date=><WeekRow key={date} date={date} empId={emp.id} forAdmin={true}/>)}
                </div>
              </div>
            </>
          )
        ...
        {view==='edit'&&editDate&&(()=>{
          ...
          return(
            <>
              <div className="page-header">
                <div><button className="back-btn" onClick={()=>setView(isAdmin?'employee':'home')}><i className="ti ti-arrow-left" aria-hidden="true"/> Back</button><div className="page-heading">Edit Times</div></div>
              </div>
              <div style={{padding:16}}>
                <div style={{fontSize:14,color:'var(--muted)',marginBottom:16}}>{emp?.name} — {dateLabel}</div>
                <div className="label">Clock In</div>
                <input type="time" value={editIn} onChange={e=>setEditIn(e.target.value)} style={{...inputStyle,marginBottom:16}}/>
                <div className="label">Clock Out</div>
                <input type="time" value={editOut} onChange={e=>setEditOut(e.target.value)} style={{...inputStyle,marginBottom:16}}/>
                <div className="label">Lunch Breaks</div>
                {editLunches.map((lb,i)=>(
                  <div key={i} style={{background:'#fff',border:'0.5px solid var(--border)',borderRadius:10,padding:12,marginBottom:8}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                      <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>START</div><input type="time" value={lb.start||''} onChange={e=>{const l=[...editLunches];l[i]={...l[i],start:e.target.value};setEditLunches(l)}} style={inputStyle}/></div>
                      <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>END</div><input type="time" value={lb.end||''} onChange={e=>{const l=[...editLunches];l[i]={...l[i],end:e.target.value};setEditLunches(l)}} style={inputStyle}/></div>
                    </div>
                  </div>
                ))}
                <button className="btn" onClick={()=>setEditLunches([...editLunches,{start:'',end:''}])} style={{marginBottom:24}}><i className="ti ti-plus" aria-hidden="true"/> Add Lunch Break</button>
                    ...
                    return(
                      <div key={emp.id} className="card report-card">
                        ...
                        <div className="week-bars">
                          {wd.map(date=>{ const r=recs.find(x=>x.date===date),w=calcWorked(r); return(<div key={date} className="week-bar-col"><div className="week-bar-day">{new Date(date+'T12:00:00').toLocaleDateString([],{weekday:'narrow'})}</div><div className="week-bar-track" style={{background:w>0?emp.color:'#E0E0E0'}}/></div>) })}
                        </div>
                      </div>
                    )
                  })}
        ...
        {view==='history'&&isAdmin&&(()=>{
          ...
          return(
            <>
              ...
              <div style={{overflowX:'auto',display:'flex',gap:8,padding:'10px 16px',borderBottom:'0.5px solid var(--border)'}}>
                {Array.from({length:12},(_,i)=>(
                  <button key={i} onClick={()=>setHistoryOffset(i)} style={{flexShrink:0,padding:'6px 14px',borderRadius:20,border:'0.5px solid #ccc',background:historyOffset===i?'#111':'#fff',color:historyOffset===i?'#fff':'#111',fontSize:12,fontWeight:historyOffset===i?600:400,cursor:'pointer',whiteSpace:'nowrap',fontFamily:'inherit'}}>
                    {weekLabel(i)}
                  </button>
                ))}
              </div>
              <div style={{padding:'10px 16px 0',fontSize:12,color:'var(--muted)'}}>Week ending {new Date(hwd[4]+'T12:00:00').toLocaleDateString([],{weekday:'long',month:'long',day:'numeric',year:'numeric'})}</div>
              <div style={{padding:'10px 16px',display:'flex',flexDirection:'column',gap:10}}>
                {employees.length===0?<div className="empty"><i className="ti ti-clock-record" aria-hidden="true"/>No employees yet.</div>:employees.map(emp=>{
                  ...
                  return(
                    <div key={emp.id} className="card" style={{padding:'12px 14px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                        ...
                        <div style={{flex:1}}><div style={{fontWeight:600,fontSize:14}}>{emp.name}</div><div style={{fontSize:12,color:'var(--muted)'}}>{days} day{days!==1?'s':''} worked</div></div>
                        <div style={{fontSize:18,fontWeight:700}}>{msToHM(total)}</div>
                      </div>
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        {hwd.map(date=>{
                          const r=empRecs.find(x=>x.date===date),w=calcWorked(r),ln=calcLunch(r)
                          return(
                            <div key={date} style={{display:'flex',alignItems:'center',fontSize:12,padding:'4px 0',borderBottom:'0.5px solid var(--border)'}}>
                              <span style={{color:'var(--muted)',width:80,flexShrink:0}}>{fmtDate(date)}</span>
                              {r?.clockIn?(
                                <>
                                  <span style={{flex:1,color:'#555'}}>{fmt(r.clockIn)} – {r.clockOut?fmt(r.clockOut):'ongoing'}</span>
                                  {ln>0&&<span style={{color:'var(--muted)',marginRight:8}}>lunch {msToHM(ln)}</span>}
                                  <span style={{fontWeight:600,minWidth:50,textAlign:'right'}}>{msToHM(w)}</span>
                                  <button onClick={()=>openEdit(emp.id,date)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:14,padding:'0 0 0 6px',lineHeight:1}}><i className="ti ti-pencil" aria-hidden="true"/></button>
                                </>
                              ):<span style={{color:'#ccc',flex:1}}>No record</span>}
                            </div>
                          )
                        })}
                      </div>
                      {total>0&&<button onClick={()=>generatePDF(emp,historyOffset).save(`${emp.name.replace(/\s+/g,'-')}-timesheet-${hwd[4]}.pdf`)} style={{marginTop:10,padding:'6px 12px',borderRadius:8,background:'var(--surface)',border:'0.5px solid #ccc',fontSize:12,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:6,color:'#111'}}><i className="ti ti-file-download" aria-hidden="true"/> Download PDF</button>}
                    </div>
                  )
                })}
          ...
          <>
            ...
            <div style={{padding:16}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:24}}>
                ...
                <button onClick={addEmployee} style={{padding:11,borderRadius:10,background:'#111',color:'#fff',border:'none',fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>Add Employee</button>
              </div>
              <div className="label">Team ({employees.length})</div>
              {employees.length===0?<div className="empty" style={{padding:'20px 0'}}><i className="ti ti-users" aria-hidden="true"/> No employees yet.</div>:(
                  ...
                  {employees.map(emp=>{
                    ...
                    return(
                      <div key={emp.id} className="card" style={{display:'flex',alignItems:'center',gap:10}}>
                        <div className="avatar" style={{background:emp.color,width:36,height:36,fontSize:12}}>{initials(emp.name)}</div>
                        <div style={{flex:1}}><div style={{fontWeight:600,fontSize:14}}>{emp.name}</div><div style={{fontSize:11,color:'var(--muted)'}}>@{empUser?.username||'—'}</div><span className="pill" style={{background:sc.bg,color:sc.color}}>{sc.label}</span></div>
                        <button onClick={()=>removeEmployee(emp.id)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:20,lineHeight:1,padding:4}}><i className="ti ti-trash" aria-hidden="true"/></button>
                      </div>
                    )
                  })}
                </div>
            ...
            <div style={{padding:16}}>
              ...
              {canUseBiometric()&&(
                <div style={{marginBottom:24}}>
                  ...
                  {hasBiometric(currentUser?.userId)?<div style={{fontSize:13,color:'#27AE60',padding:'10px 0'}}>✓ Face ID / fingerprint enabled on this device</div>:(
              ...
              {isAdmin&&(
                <>
                  ...
                  <button className="btn btn-primary" onClick={()=>{saveSettings({...settings,email:settEmail});showToast('Email saved!')}} style={{marginBottom:24}}><i className="ti ti-check" aria-hidden="true"/> Save Email</button>
                  <div className="label">Data management</div>
                  <button className="btn btn-danger" onClick={()=>{if(confirm('Clear ALL time records?')){saveRecords([]);showToast('Records cleared')}}} style={{marginBottom:16}}><i className="ti ti-trash" aria-hidden="true"/> Clear All Time Records</button>
                </>
              )}
              {!isAdmin&&(
                <>
                  <div className="label">My Records</div>
                  <button className="btn btn-danger" onClick={clearMyWeek} style={{marginBottom:16}}><i className="ti ti-calendar-x" aria-hidden="true"/> Clear My Week Records</button>
                </>
              )}
